...
const POST = async ({ request }) => {
  ...
  if (text) {
    ...
    if (matchedLicenses.length > 0) {
      ...
      const processedBody = bestMatch.body.replace(/\[fullname\]/g, name).replace(/\[year\]/g, currentYear);
    ...
    const processedLicenses = matchedLicenses.map((license) => {
      const processedBody = license.body.replace(/\[fullname\]/g, name).replace(/\[year\]/g, currentYear);
